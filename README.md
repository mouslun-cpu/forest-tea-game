...
用手機體驗決策樹與隨機森林。學生全程是手搖店店長，任務是根據客人紀錄，預測客人「會不會加點心」。老師建立房間、投影 QR，學生用暱稱加入。

預設入門約 15 分鐘：看 8 筆舊紀錄 → 選一個問題長出小樹 → 每人抽不同紀錄與候選問題 → 全班的樹投票 → 用 6 位新客人檢查成果 → 三題回顧。進階約 30 分鐘，另外加入 Bagging、最多兩層樹與 AdaBoost 錯題接力。時間為建議節奏，由老師控制。

**30 秒開場：**「今天你是手搖店店長，要猜新客人會不會加點心。先看 8 筆舊紀錄，選一個問題，系統就會幫你分組、數多數，長出一棵小樹。接著每人看不同紀錄、選不同問題，讓全班的小樹一起投票。你只要選問題、看結果、按提交。卡片都是教學編寫的客人。」
...
- 預設 15 分鐘入門、可選 30 分鐘進階；倒數只提醒，由老師收件與揭曉。
- 8 筆生活化舊紀錄，線索為肚子餓、想吃甜、有折扣、外帶；全為教學合成案例。
- 入門每棵樹只選一個問題，系統分組、計算各組多數，學生不用手算或畫樹。
- 森林階段自動抽紀錄、提供隨機候選問題，再合併已提交樹的預測。
- 進階另有 Bagging 與最多三輪 AdaBoost；無有效弱模型或完美模型會提前停止。
- 6 位獨立新客人；課堂成果與可收合的系統自動建樹示範分開呈現。
- 老師當下操作提示、30 秒講稿、名單、QR、獨立投影與 CSV／Markdown 匯出。
- 學生三題回顧、徽章、草稿與身分恢復；版本檢查避免跨輪提交。
...
目前玩法與帶課講稿在 `docs/game-plan.md`；實作採原生 CSS 視覺系統及本機／Firebase server adapter。
