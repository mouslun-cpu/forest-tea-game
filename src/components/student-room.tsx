...
import { avatars, featureInfo, features, phaseName, questions, stages } from '@/lib/presentation';
...
export default function StudentRoom({ code }: { code: string }) {
  ...
  return <main className="student-app"><header className="student-topbar"><Brand small/><SoundToggle/><button className="icon-button" aria-label="查看玩法提示" onClick={()=>setHelp(true)}><BookOpen size={19}/></button></header>
    <div className="player-bar"><span className="player-avatar">{avatars[player.avatar % avatars.length]}</span><div><strong>{player.name}<small>見習店長</small></strong><div className="xp-track"><span style={{width:`${level/7*100}%`}}/></div></div><span className="xp-count"><Star size={15} fill="currentColor"/> {level} / 7</span></div>
    <div className="mobile-stage-track">{stages.map((s,i)=><div key={s.phase} className={`${room.phase === s.phase ? 'active' : ''} ${player.trees[s.phase as Method] || (s.phase==='boosting' && Object.keys(player.proposals).length>0) ? 'complete' : ''}`}><span>{s.icon}</span><small>{['小樹','訂單袋','森林','接力'][i]}</small></div>)}</div>
    <ErrorNotice message={error}/>
    <div className="connection-line"><span><Wifi size={12}/> {error ? '連線重試中' : `小店 ${code}`}</span><StudentTimer room={room}/></div>
    ...
    <div className="student-content" key={`${room.phase}-${room.round}`}>
      ...
      {['tree','bagging','forest'].includes(room.phase) && <TreeMission room={room} busy={busy} act={act}/>}
...
function Waiting({room}:{room:RoomView}) {return <section className="waiting-mission"><span className="eyebrow">YOUR APRON IS READY</span><h1>圍裙穿好了！</h1><p>小店準備開張，等老師開始冒險。</p><div className="waiting-scene"><img src="/art/hero-shop.svg" alt="等待開張的森林小店"/><div className="waiting-bubble">等一下，點心馬上烤好 🍪</div></div><div className="mission-brief"><span>📮</span><div><strong>今天的任務</strong><p>看看過去的訂單，猜猜下一位客人<br/>會不會加點心。</p></div></div><div className="waiting-friends">{room.players.slice(0,8).map(p=><span key={p.id} title={p.name}>{avatars[p.avatar%6]}</span>)}<small>{room.players.length} 位店長已就位</small></div></section>;}
...
function TreeMission({room,busy,act}:{room:RoomView;busy:boolean;act:Act}) {
  ...
  return <>
    {method!=='tree'&&!drawn&&!locked ? <section className="draw-mission panel"><div className="bag-illustration">{method==='forest'?'🌳':'🧺'}<span>24</span></div><h2>{method==='forest'?'同一袋訂單，新的看法':'抽一袋，你的練習訂單'}</h2><p>{method==='forest'?'這次每個分岔只能從兩個線索選。看看你會長出什麼樹。':'從大家共同的訂單中，抽出 24 次。抽完放回去，所以有些客人會再出現！'}</p><button className="button primary full" onClick={()=>{setDrawn(true);setBagOpen(true);playChime();}}>{method==='forest'?'打開我的線索袋':'抽我的訂單袋'}<Sparkles size={18}/></button></section> : <>
...
function BoostMission({room,busy,act}:{room:RoomView;busy:boolean;act:Act}) {
  ...
  return <><div className="boost-rounds">{[0,1,2].map(i=><div className={i===room.round?'active':i<room.round?'complete':''} key={i}><span>{i<room.round?<Check size={16}/>:i+1}</span><small>{['先抓方向','照顧錯題','再補盲點'][i]}</small>{i<2&&<ArrowRight size={16}/>}</div>)}</div><div className="mission-brief"><span>⚡</span><div><strong>{room.round===0?'每位客人，一樣重要':'上一輪漏掉的客人，現在更重要'}</strong><p>{room.round===0?'選一條簡單的規則，當我們的第一個幫手。':'關注籌碼越多，這張訂單猜錯的代價越大。再找一個幫手吧。'}</p></div></div><button className="order-bag-button" onClick={()=>setOrdersOpen(true)}><span>🪙</span><div><strong>查看訂單與關注籌碼</strong><small>24 張都還在，只是重點不同了</small></div><Eye size={19}/></button>
...
function FinalMission({room}:{room:RoomView}) {
  ...
  return <section className="final-mission"><div className="final-art"><img src="/art/forest-badge.svg" alt="森林徽章"/></div><span className="eyebrow">THE FINAL CHALLENGE</span><h1>真正的新客人，來了！</h1><p>把練習本收起來。<br/>用 12 張沒看過的訂單，考驗大家的規則。</p>{!room.revealed?<div className="panel"><LockKeyhole size={28}/><h2>答案還在信封裡</h2><p>先猜猜，哪一種方式會表現最好？<br/>這只是你的預想，不計分。</p><div className="prediction-options">{['一棵樹','分頭投票','隨機森林','錯題接力'].map(name=><button className={`button ${predicted===name?'primary':'secondary'}`} key={name} onClick={()=>setPredicted(name)}>{name}</button>)}</div>{predicted&&<p className="tiny-note">你看好「{predicted}」，等老師打開信封！</p>}</div>:<><Done title="新客人的答案揭曉了！">看看規則面對沒看過的訂單，表現有什麼不同。</Done><div className="panel results-list">{room.results?.filter(r=>!r.name.includes('系統示範')).map((row,i)=><div className="result-row" key={row.name}><span className="result-icon">{['🌱','🧺','🌳','⚡'][i%4]}</span><div><strong>{row.name}</strong><div className="result-bar"><span style={{width:`${row.correct/row.total*100}%`}}/></div><small>{row.modelCount} 個模型</small></div><b>{row.correct}<small> / {row.total}</small></b></div>)}</div><div className="mission-brief"><span>💭</span><div><strong>這場結果，有讓你意外嗎？</strong><p>很多人一起想，也可能一起犯錯。這是本場體驗，不代表某種方法永遠最好。</p></div></div></>}</section>;
}
...
function Graduation({room}:{room:RoomView}) {const me=room.me!;return <section className="graduation"><div className="confetti" aria-hidden="true">✦　✧　✦　✧</div><img src="/art/forest-badge.svg" alt="森林店長徽章"/><span className="eyebrow">YOU MADE THE FOREST GROW</span><h1>辛苦了，{me.name}店長！</h1><p>你的小小判斷，<br/>今天長成了一整片森林。</p><div className="panel graduation-card"><Trophy size={28}/><h2>{me.answers?'森林店長 · 挑戰完成':'森林夥伴 · 今日參與'}</h2><p>一串問題，是一棵樹。<br/>分頭學再合併，是 Bagging。<br/>加入隨機線索，一起長成森林。<br/>一輪接一輪補問題，是 Boosting。</p>{me.answers&&<span className="pill">概念挑戰 {me.answers.filter((a,i)=>a===questions[i].answer).length} / 3</span>}</div><Link href="/" className="button primary full">回到森林小店<ArrowRight size={18}/></Link></section>;}
