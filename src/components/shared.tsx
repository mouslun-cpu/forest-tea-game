...
export function TreeView({node, compact = false, order}: {node: TreeNode; compact?: boolean; order?: Order}) {
  if (!node.feature) return <div className={`tree-leaf ${node.prediction ? 'yes' : ''}`}>{node.prediction ? '🍪 預測會加點' : '🥤 預測不加點'}{!compact && <small>{node.count} 筆訂單 · {node.yesCount} 筆加點心</small>}</div>;
...
export function Done({title = '你的規則已送出！', children}: {title?:string; children?:React.ReactNode}) { return <div className="done-banner"><span><Check size={20}/></span><div><strong>{title}</strong><p>{children ?? '你的小樹已經準備好。等老師揭曉，一起看看！'}</p></div></div>; }
