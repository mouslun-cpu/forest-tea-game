...
import { avatars, featureInfo, phaseName, questionsFor, stagesFor } from '@/lib/presentation';
...
export function TeacherRoom({ code, displayOnly = false }: { code: string; displayOnly?: boolean }) {
  ...
  const control = (action: Control) => {
    if (action === 'end' || action === 'reset' || ((action === 'close' || action === 'reveal') && collecting && room.open && pending > 0)) setConfirm(action);
  ...
  return <main className={`teacher-shell ${displayOnly ? 'display-only' : ''}`}>
    ...
    <div className="teacher-layout"><div className="teacher-main">
      ...
      {room.phase === 'final' && !room.revealed && <section className="panel"><p className="eyebrow">SEALED ORDERS</p><h2>🔒 6 位新客人，等待揭曉</h2><p>這些答案沒有參與抽樣、選規則或調整模型。所有方法都會接受同一份挑戰。</p><p>{room.duration === 'short' ? '先猜猜看：一棵樹和全班的森林，誰會猜對比較多？投票不保證每次都更準。' : '先猜猜看：分頭投票和錯題接力，這次會有什麼不同？多個模型不保證更準。'}</p></section>}

      {resultsVisible && <section className="panel"><div className="teacher-stage-heading"><div><p className="eyebrow">TODAY’S LITTLE DISCOVERY</p><h2>新客人挑戰，成績揭曉</h2></div>{!displayOnly && <div><button className="button secondary" onClick={() => downloadResults(room, 'csv')}><Download size={16}/>CSV</button><button className="button ghost" onClick={() => downloadResults(room, 'md')}>Markdown</button></div>}</div><details><summary>老師補充：切換系統自動建樹示範</summary><div className="teacher-controls" role="group" aria-label="成果類別"><button className={`button ${resultTab === 'class' ? 'primary' : 'secondary'}`} aria-pressed={resultTab === 'class'} onClick={() => setResultTab('class')}>課堂體驗</button><button className={`button ${resultTab === 'system' ? 'primary' : 'secondary'}`} aria-pressed={resultTab === 'system'} onClick={() => setResultTab('system')}>固定條件示範</button></div></details><p>{resultTab === 'class' ? '使用本場已鎖定的手作規則；單樹列取一位已提交店長作例子，並非全班平均。' : room.duration === 'short' ? '系統自動選問題，每棵樹只問一個問題，森林有 5 棵樹。這些分數獨立於學生的手作模型。' : '系統自動選分裂、固定種子、最多深度 2；Bagging 與森林各 5 棵樹。這些分數獨立於學生的手作模型。'}</p><div className="results-list">{resultRows.map(row => <article className="result-row" key={row.name}><div><h3>{row.name}</h3><small>{row.modelCount} 個模型{row.modelCount === 0 ? ' · 未建立模型，僅為預設預測' : ''}</small></div><div><strong>{row.modelCount ? `${row.correct}／${row.total}` : '未參與'}</strong><small>{row.modelCount ? '張答對' : '不比較成績'}</small></div><div className="progress-track"><span style={{ width: `${row.modelCount && row.total ? row.correct / row.total * 100 : 0}%` }}/></div></article>)}</div><p>這是本場少量教學模擬題的結果，不能由此認定某種演算法普遍更好。</p><details><summary>查看封存訂單與答案</summary><div className="mini-tree-grid">{room.testOrders?.map(order => <OrderCard key={order.id} order={order}/>)}</div></details></section>}

      {['reflection', 'ended'].includes(room.phase) && <section className="panel"><p className="eyebrow">TAKE A LITTLE WISDOM HOME</p><h2>{room.phase === 'ended' ? '今天的小店，圓滿打烊' : '三個問題，把觀念帶回家'}</h2><p>{room.reflections?.answered ?? 0} 位店長已完成離店任務。</p>{questions.map((question, i) => <article key={question.text}><h3>{i + 1}. {question.text}</h3>{(room.phase === 'ended' || !displayOnly) && <><p>{question.options[question.answer]}。{question.explanation}</p><small>{room.reflections?.correct[i] ?? 0}／{room.reflections?.answered ?? 0} 人答對</small></>}</article>)}<p>{room.duration === 'short' ? '選一個問題，把客人分組，就是一棵小樹。每棵樹看到不同紀錄、考慮不同問題，再一起投票，就是今天的森林體驗。' : '一串問題是一棵樹；分頭學再合併是 Bagging；加入分岔線索抽樣形成森林；前後相接補問題是 Boosting。'}</p>{room.phase === 'ended' && !resultsVisible && <p>這堂課在最終揭曉前結束，尚無已揭露的封存題成果。</p>}</section>}
    </div>

    {!displayOnly && <aside className="teacher-sidebar"><section className="panel"><p className="eyebrow">TEACHER’S COUNTER</p><h2>老師下一步</h2><p>{room.phase === 'waiting' ? '學生加入後，念下方 30 秒開場，再按「開始第一關」。' : room.phase === 'ended' ? '課堂已完成，可匯出成果。' : room.phase === 'reflection' ? '請學生回答手機上的三題，再完成課堂。' : room.phase === 'final' ? '先請大家猜單樹或森林誰會更準，再揭曉。' : room.revealed ? '一起看客人走過哪條路、票數怎麼合併，再前往下一階段。' : '請學生完成手機任務。看到提交人數後，按「收件並揭曉」。'}</p><div className="teacher-controls">{room.phase !== 'ended' && <button className="button primary" disabled={busy || (collecting && !room.submitted)} onClick={() => control(primaryAction)}>{busy ? '處理中…' : primaryText}</button>}{collecting && room.open && <button className="button secondary" disabled={busy || !room.submitted} onClick={() => control('close')}>先收件，暫不揭曉</button>}{room.phase !== 'ended' && <><button className="button secondary" disabled={busy} onClick={() => control(room.endsAt === null ? 'resume' : 'pause')}>{room.endsAt === null ? <Play size={16}/> : <Pause size={16}/>} {room.endsAt === null ? '開始／繼續倒數' : '暫停倒數'}</button><button className="button ghost" disabled={busy} onClick={() => control('addTime')}>＋ 加 1 分鐘</button></>}</div><p>倒數不會自動收件。只有已提交的模型參與合併。</p></section><section className="panel"><p className="eyebrow">A NOTE FOR YOU</p><h3>{room.phase === 'waiting' ? '30 秒開場，照著念就好' : '這一站，帶大家發現什麼？'}</h3><p>{stage?.note ?? (room.phase === 'waiting' ? '今天你是手搖店店長，要猜新客人會不會加點心。先看 8 筆舊紀錄，選一個問題，系統就會幫你分組、數多數，長出一棵小樹。接著每人看不同紀錄、選不同問題，讓全班的小樹一起投票。你只要選問題、看結果、按提交。卡片都是教學編寫的客人。' : room.phase === 'final' ? '先收集學生對結果的預期，再揭曉共同封存題。問：哪些模型可能一起犯錯？不要把本場名次當作演算法優劣定論。' : room.duration === 'short' ? '請學生說出：樹根據什麼猜？森林怎麼合併？為什麼還要用新客人檢查？' : '請學生用自己的話說明平行投票與循序糾錯，留意是否誤把人氣票選當作 AdaBoost。')}</p></section><section className="panel"><h3>課堂工具</h3><div className="teacher-controls"><button className="button secondary" onClick={() => setModal('roster')}>查看提交名單</button><button className="button secondary" onClick={() => setModal('qr')}>放大加入 QR</button>{['waiting', 'tree', 'bagging', 'forest', 'boosting'].includes(room.phase) && <button className="button ghost" disabled={busy} onClick={() => control('demo')}>加入系統示範店長</button>}{room.phase !== 'ended' && <button className="button danger" disabled={busy} onClick={() => control('end')}>提前結束課堂</button>}<button className="button ghost" disabled={busy} onClick={() => control('reset')}>重開一間新店</button></div><small>{room.storage === 'local' ? '本機儲存模式' : 'Firebase 雲端儲存'} · {demoCount} 位系統示範</small></section></aside>}
    </div>
